...
# print('Roughly the number of unique words: {}'.format(len(set(word for word in source_text.split()))))

sentences = source_text.split('\n')
word_counts = [len(sentence.split()) for sentence in sentences]
print('Number of sentences: {}'.format(len(sentences)))
print('Average number of words in a sentence: {}'.format(np.average(word_counts)))
# print('Average number of words in a sentence: {}'.format(np.mean(word_counts)))
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    enc_cell = tf.contrib.rnn.MultiRNNCell([make_lstm(rnn_size) for _ in range(num_layers)])
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    def make_lstm(rnn_size):
        return tf.contrib.rnn.BasicLSTMCell(rnn_size)
    dec_cell = tf.contrib.rnn.MultiRNNCell([make_lstm(rnn_size) for _ in range(num_layers)])
    ...
    for epoch_i in range(epochs):
        for batch_i, (source_batch, target_batch) in enumerate(
            ...
            print('Epoch {:>3} Batch {:>4}/{} - Train Accuracy: {:>6.3f}, Validation Accuracy: {:>6.3f}, Loss: {:>6.3f}'
                  .format(epoch_i+1, batch_i+1, len(source_int_text) // batch_size, train_acc, valid_acc, loss))
